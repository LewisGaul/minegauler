...
def get_neighbours(coord, dims, include=False):
    x, y = coord
...
class Gui(object):
    ...
    def make_panel(self):
        ...
        self.face_image = PhotoImage(name='face',
            file=osp.join(im_direc, 'faces', 'ready1face.ppm'))
    ...
    def get_images(self):
        ...
        if not osp.isfile(osp.join(im_path, '1flag%02d.ppm'%im_size)):
            im = PILImage.open(osp.join(im_path, '1flag.png'))
            data = np.array(im)
            ...
            im.save(osp.join(im_path, '1flag%02d.ppm'%im_size))
            im.close()
        self.flag_image = PhotoImage(name='flag',
            file=osp.join(im_path, '1flag%02d.ppm'%im_size))
    ...
    def show_text(self, filename, width=80, height=24, title=None):
        ...
        if osp.isfile(osp.join(data_direc, filename + '.txt')):
            with open(osp.join(data_direc, filename + '.txt'), 'r') as f:
...
class NrConfig(object):
    def __init__(self, grid, density=0.2, infinite=False):
        self.grid = grid
