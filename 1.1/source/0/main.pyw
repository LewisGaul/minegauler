...
class Gui(object):
    ...
    def save_game_data(self):
        ...
        if not self.name and self.game.state == WON:
            ...
            highscores = [h for h in self.highscores[:] if h['name'] == self.name]
    ...
    def close_root(self):
        ...
        self.root.destroy()
        with open(osp.join(main_direc, 'settings.txt'), 'w') as f:
...
if __name__ == '__main__':
    try:
        with open(osp.join(main_direc, 'settings.txt'), 'r') as f:
