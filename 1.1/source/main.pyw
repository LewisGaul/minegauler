...
from PIL import Image as PILImage, ImageTk as TkImage
...
class Gui(object):
    def __init__(self, settings=dict()):
        ...
        self.root.iconbitmap(default=join(im_direc, 'icon.ico'))
    ...
    def get_images(self):
        ...
        for n in [i for i in range(1, 4) if not isfile(
    ...
    def left_press(self, event=None, coord=None):
        ...
        if self.right_button_down:
            ...
            if (self.drag_select and self.game.state != CREATE and
                b.state == READY):
                self.click_button(b)
    ...
    def left_release(self, event=None, coord=None):
        ...
        if self.right_button_down:
            ...
            self.mouse_down_coord = None
            if self.game.state in [ACTIVE, READY] and b.state == READY:
                self.click_button(b)
                if self.game.state == ACTIVE: # Still active
                    n = min(self.game.lives_remaining, 3)
                    self.face_button.config(image=self.face_images['ready%sface'%n])
            elif self.game.state == CREATE and b.state in [READY, SAFE]:
                self.create_click(b)
    ...
    def create_rightclick(self, button):
        ...
        if b.state == READY:
            ...
            if n < self.per_cell:
                ...
                self.game.grid.itemset(b.coord, -BIG)
                b.state = READY
        elif b.state == SAFE:
    ...
    def set_mines_counter(self):
        ...
        nr_remaining = self.mines - np.where(grid<-9, -grid/10, 0).sum()
    ...
    def toggle_timer(self, event=None):
        ...
        if (self.timer_hide_var.get() and
            self.game.state in [READY, ACTIVE, INACTIVE]):
    ...
    def update_settings(self, run=False, new=False):
        ...
        if (self.first_success_var.get() and not self.first_success and
            ...
            self.game = Game(self.settings, False)
        self.first_success = self.first_success_var.get()
    ...
    def new(self, event=None, new=True):
        ...
        self.prob_label.place_forget()
        if self.game.state == CREATE:
            self.game.grid = -BIG * np.ones(self.dims, int)
            self.set_mines_counter()
            return
    ...
    def toggle_creation(self, reset=True):
        if self.create_var.get(): # Only implement if create_var is true.
            ...
            field = Minefield(self.settings, create=False)
            field.origin = KNOWN
            grid = (self.game.grid!=-BIG) * (self.game.grid<0)
    ...
    def auto_flag(self, event=None):
        if (self.lives > 1 or self.per_cell > 1 or self.detection != 1 or
            self.distance_to or self.game.state not in [ACTIVE, COLOURED]):
            return
        grid = self.game.grid.copy()
        grid = np.where(grid<-9, -BIG, grid)
        probs = NrConfig(grid, mines=self.mines).all_probs
        if not ((probs!=10) * (grid==-10)).any():
        ...
        for coord in [c for c in get_nonzero_coords(probs==1) if
            ...
            b.state = FLAGGED
            self.game.grid.itemset(coord, -10)
        self.set_mines_counter()
    ...
    def set_difficulty(self, new=True):
        self.submit_name_entry()
