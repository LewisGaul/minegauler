...
class Minefield(object):
    def __init__(self, mine_coords=None, auto_create=True, **kwargs):
        ...
        for s in ['diff', 'dims', 'mines', 'per_cell']:
    ...
    def generate_from_list(self, coords):
        ...
        self.mines = len(set(self.mine_coords))
    ...
    def generate_rnd(self, open_coord=None):
        ...
        self.mine_coords = sorted(avble_coords[:self.mines])
        # Create empty numpy array of integers to represent mine positions.
        self.mines_grid = np.zeros(self.dims, int)
        # Assign a one to the mines_grid where the mines should be.
        for c in set(self.mine_coords):
            self.mines_grid.itemset(c, self.mine_coords.count(c))
    ...
    def serialise(self, path):
        ...
        for attr in ['diff', 'dims', 'mines', 'per_cell']:
...
class Game(object):
    ...
    def get_time_passed(self):
        ...
        if not self.start_time:
            ...
            return self.finish_time - self.start_time - self.pause_time
        else:
            return tm.time() - self.start_time - self.pause_time
    ...
    def get_prop_flagged(self):
        """Calculate the proportion of mines which are flagged."""
        if self.flags is None:
            raise ValueError('Unkown number of flags.')
        else:
            return float(self.flags) / self.mines
