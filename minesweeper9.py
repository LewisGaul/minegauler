...
detection_mines = {
            ('b', 0.5): 10, ('b', 1): 10, ('b', 1.5): 8,
            ('b', 1.8): 7, ('b', 2): 6, ('b', 2.2): 5,
            ('b', 2.5): 5, ('b', 2.8): 4, ('b', 3): 4,
            ('i', 0.5): 40, ('i', 1): 40, ('i', 1.5): 30,
            ('i', 1.8): 25, ('i', 2): 25, ('i', 2.2): 20,
            ('i', 2.5): 18, ('i', 2.8): 16, ('i', 3): 16,
            ('e', 0.5): 99, ('e', 1): 99, ('e', 1.5): 85,
            ('e', 1.8): 70, ('e', 2): 65, ('e', 2.2): 50,
            ('e', 2.5): 45, ('e', 2.8): 40, ('e', 3): 40}
...
class Minefield(object):
    ...
    def create(self, mines='default', overwrite=False, prop=None):
        ...
        #Only works for 2D.
            ...
            self.mines_grid = np.array(mines)
            self.per_cell = max(self.mines_grid.max(), self.per_cell)
            self.shape = self.mines_grid.shape
            self.size = self.mines_grid.size
            #Stop the given grid being overwritten below.
            mines = -1
        elif type(mines) in [set, list]:
            #Assumes the given list or set contains the coordinates of mines.
            for c in list(mines):
        ...
        elif mines not in range(1, self.size*self.per_cell) and self.shape in shape_difficulty:
            mines = detection_mines[(shape_difficulty[self.shape], self.detection)]
        elif mines not in range(1, self.size*self.per_cell):
            d = float(self.detection) - 1
            prop = 0.2 * (0.09*d**3 - 0.25*d**2 - 0.15*d + 1)
            mines = int(round(self.size*prop, 0))

        #If the grid of mines isn't already determined.
        if mines in range(1, self.size*self.per_cell):
            if self.per_cell == 1:
                #This is quicker? But only works for one per cell.
                ...
                while self.mines_grid.sum() < mines:
                    cells = np.random.randint(0, self.size, mines - self.mines_grid.sum()).tolist()
                    for i in cells:
                        self.mines_grid.itemset(i, min(self.per_cell, cells.count(i)))
        ...
        if self.per_cell > 1:
            ...
            for c in self.mine_coords:
                if self.mines_grid.item(c) > 1:
                    repeats += [c]*(self.mines_grid.item(c) - 1)
...
class Game(Minefield):
    ...
    def get_images(self):
        im_size = self.button_size - 6
        #If the .ppm files for the current size do not exist, create them from th .png file (mines first).
    ...
    def make_button(self, coord):
        f = Frame(self.mainframe, width=self.button_size, height=self.button_size)
        self.frames[coord] = f
        ...
        btext = StringVar()
        b = Button(f, bd=3, takefocus=0, textvariable=btext, font=self.cellfont, command=self.leftclick)
        b.text = btext
        b.coord = coord
        b.image = ''
        b.grid(sticky='nsew')
        b.bind('<Button-3>', self.rightdown)
        b.bind('<Double-Button-1>', self.doubleleftclick)
        b.bind('<Button-1>', self.leftdown)
        b.bind('<ButtonRelease-3>', self.rightrelease)
        b.bind('<ButtonRelease-1>', self.leftrelease)
        b.bind('<B1-Motion>', self.motion('left'))
        b.bind('<B3-Motion>', self.motion('right'))
    ...
    def show_info():
        ...
        if not self.start:
            ...
            self.detection = detection_options[detection_var.get()]
        self.focus = info_window = Toplevel(self.root)
    ...
    def change_shape():
        ...
        if difficulty_var.get() == 'c':
            ...
            customise_window = Toplevel(self.root)
    ...
    def choose_lives():
        ...
        def get_lives(event):
            try:
                ...
                self.lives = 1
            self.lives_remaining_var.set(self.lives)
            self.lives_var.set(self.lives if self.lives in [1, 2, 3] else -1)
            lives_window.destroy()
            if self.game_state != 'lost':
                n = min(3, self.lives_remaining_var.get())
                face_button['image'] = self.face_images['ready%sface'%n]
        lives_window = Toplevel(self.root)
