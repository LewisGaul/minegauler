...
class Gui(BasicGui):
    def __init__(self, settings=dict()):
        ...
        data_path = join(direcs['data'], 'data.txt')
        datacopy_path = join(direcs['data'], 'datacopy.txt')
        ...
        try:
            ...
            if data != self.all_data:
                with open(join(direcs['data'], 'corrupt data {}.txt'.format(
            ...
            copy_file(data_path,
                join(direcs['data'], 'data recovery{}.txt'.format(
                    tm.asctime().replace(':', ''))))
            self.all_data = []
    ...
    def make_name_entry(self):
        ...
        self.name_entry = Entry(self.root, bd=2, width=self.width,
            font=self.msg_font, justify=CENTER, disabledforeground='black')
        if self.name:
    ...
    def left_release(self, event=None, coord=None):
        ...
        if (self.game.state in [LOST, WON, INACTIVE] or
            not self.mouse_coord):
            return
        b = self.buttons[self.mouse_coord]
        if self.right_button_down:
            self.both_release()
        elif not self.is_both_click and not self.drag_select:
            self.mouse_coord = None
    ...
    def right_release(self, event=None):
        self.right_button_down = False
        if self.game.state in [LOST, WON, INACTIVE] or not self.mouse_coord:
            return
        if self.left_button_down:
            self.both_release()
        else:
    ...
    def both_press(self):
        self.is_both_click = True
        neighbours = get_neighbours(self.mouse_coord, self.dims, self.detection, True)
    ...
    def motion(self, event):
        ...
        all_coords = [(u, v) for u in range(self.dims[0])
            for v in range(self.dims[1])]
        if cur_coord in all_coords and cur_coord != self.mouse_coord:
            if (self.left_button_down and not self.right_button_down
                and not self.is_both_click): #left
                if self.mouse_coord:
                    old_button = self.buttons[self.mouse_coord]
                ...
                if not self.mouse_coord:
                    ...
                    return
                if self.mouse_coord:
                    old_neighbours = get_neighbours(self.mouse_coord,
        ...
        elif cur_coord != self.mouse_coord and self.mouse_coord:
            if self.left_button_down and not self.right_button_down: #left
                button = self.buttons[self.mouse_coord]
    ...
    def save_game_data(self):
        ...
        if not self.name and self.game.state == WON:
            ...
            if self.lives == 1 and highscores[0] == entry:
                ...
                self.game.serialize(
                    join(direcs['main'], 'boards', 'highscores', fname))
    ...
    def close_root(self):
        ...
        # with open(join(direcs['data'], 'highscores.txt'), 'w') as f:
        #     json.dump(self.all_highscores, f)
        with open(join(direcs['data'], 'data.txt'), 'w') as f:
            json.dump(self.all_data, f)
    ...
    def save_board(self):
        ...
        options = {
            ...
            'initialdir': join(direcs['main'], 'boards', 'saved'),
            'initialfile': fname,
            'parent': self.root,
    ...
    def show_highscores(self, event=None, htype='time', flagging=None):
        ...
        def set_name(event):
            ...
            with open(join(direcs['data'], 'datacopy.txt'), 'w') as f:
                json.dump(self.all_data, f)
            copy_file(join(direcs['data'], 'datacopy.txt'),
                join(direcs['data'], 'data.txt'))
            for w in window.children.values():
                w.destroy()
