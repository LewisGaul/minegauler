...
UNCLICKED = -401 # Must match other scripts
MINE = -410
...
class Minefield(object):
    def __init__(self, settings, mines_list=None, block_create=False):
        self.settings = dict()
    ...
    def get_mine_coords(self):
        """Get a list of the coordinates of the mines using mines grid."""
...
class Game(object):
    ...
    def serialize(self, path):
        if not hasattr(self.minefield, 'mine_coords'):
            grid = (self.grid!=UNCLICKED) * (self.grid<0)
