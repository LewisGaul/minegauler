...
def create_windows_wrapper(
    ...
    *,
    ...
    dest = Path(dest).with_suffix(".bat")
...
def create_package(from_dir: Path, dest_dir: Path, fmt: Format) -> str:
    # Create outer files.
    ...
    if Platform.current() is Platform.WINDOWS:
        ...
        os.symlink(
            os.path.join(APP_NAME, "minegauler-bot"), from_dir / "minegauler-bot"
        )

    with open("package/README.txt.template", "r", encoding="utf-8") as f:
        readme = f.read().format(
            root_filename=root_filename, dist_filename=dist_filename
        )
    with open(from_dir / "README.txt", "w", encoding="utf-8") as f:
        f.write(readme)
...
def main(argv: list[str]) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="pyinstaller-") as tmpdir:
        ...
        run_pyinstaller(tmpdir_path)
        path = create_package(tmpdir_path, args.output_dir, args.format)
