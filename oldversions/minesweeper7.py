...
detection_mines = {
            ('b', 0.5): 10, ('b', 1): 10, ('b', 1.5): 7, ('b', 1.8): 6,
...
class Game(Minefield):
    ...
    def play(self, mines='default', first_success=False):
        ...
        def show_highscores(settings=None, flagging=None, htype='Time', window=None):
            ...
            if self.visitor:
                highscores = reduce(lambda x, y: x + ([y] if (y['Name'] or y==self.highscores[settings][-1]) and y['Name'] not in map(lambda d: d['Name'], x) else []), highscores, [])

            if highscores and self.highscores[settings][-1]['Mine coords'] == self.mine_coords:
                current = True
            else:
                current = False
        ...
        def set_zoom():
            ...
            def get_zoom(event=None):
                global cellfont
                if event:
                    self.button_size = int(event.widget.get())
                else:
                    self.button_size = 16
                if self.button_size == 16:
                    ...
                    zoom_var.set(True)
                cellfont = ('Times', (10*self.button_size)/17, 'bold')
                for f in frames.values():
                    f.config(height=self.button_size, width=self.button_size)
                    f.children.values()[0]['font'] = cellfont
                window.destroy()
        ...
        new_button_frame.pack(side='left')
        Button(new_button_frame, bd=2, textvariable=lives_remaining_var, command=new).grid(sticky='nsew')
        timer_var = StringVar()
        timer_var.set("000")
        ...
        def leftclick(coord=None, actual_click=True):
            ...
            if create_var.get() and self.grid[coord] >= -1:
                ...
                #Assumes you can't create an 'Entry' widget during a game.
                try:
                    if self.focus.bindtags()[1] == 'Entry':
                        self.focus.focus_set()
                        return
                ...
                if self.grid[coord] == -1 and self.game_state in ['ready', 'active']:
                    ...
                    #Otherwise a mine is hit.
                        ...
                        for i in ['mine', 'redmine', 'bluemine']:
                            im_path = os.path.join(directory, 'Images', i)
                            if not os.path.isfile(im_path + str(im_size) + '.ppm'):
                                #Image() is from PIL, used to convert/resize the image.
                                im = Image.open(im_path + '.png')
                                im = im.resize(tuple([im_size]*2), Image.ANTIALIAS)
                                im.save(im_path + str(im_size) + '.ppm')
                        ...
                        if lives_remaining_var.get() == 0:
                            ...
                            for c in buttons:
                                ...
                                if (self.grid[c] < -1 and self.grid[c] != self.final_grid[c]):
                                    ...
                                    b1.image = PhotoImage(file=os.path.join(directory, 'Images', 'mine' + str(im_size) + '.ppm'))
                                    b1['image'] = b1.image
                            b.image = PhotoImage(file=os.path.join(directory, 'Images', 'redmine' + str(im_size) + '.ppm'))
                            b['image'] = b.image
                            ...
                            b.image = PhotoImage(file=os.path.join(directory, '..', 'Images', 'bluemine' + str(im_size) + '.ppm'))
                            b['image'] = b.image
        ...
        def make_button(coord, bindings=True):
            ...
            btext = StringVar()
            b = Button(f, bd=3, takefocus=0, textvariable=btext, font=cellfont, command=leftclick)
