...
class Game(Minefield):
    def __init__(self, play=True):
        ...
        self.visitor = False
        self.settings = {'per_cell': self.per_cell, 'first_success': self.first_success, 'mines': self.mines, 'detection': self.detection, 'shape': self.shape, 'difficulty': self.difficulty, 'lives': self.lives, 'drag_click': self.drag_click, 'visitor': self.visitor, 'button_size': self.button_size}
        #Necessary??
    ...
    def play(self, mines='default', first_success=False):
        ...
        def show_highscores(settings=None, flagging=None, htype='Time', window=None):
            ...
            if self.visitor:
                highscores = reduce(lambda x, y: x + ([y] if y['Name'] not in map(lambda d: d['Name'], x) else []), highscores, [])
            ...
            if self.visitor and self.focus.bindtags()[1] != 'Entry' and current:
                d = self.highscores[settings][-1]
                font = ('Times', 10, 'bold')
            ...
            if create_var.get() and self.grid[coord] >= -1:
                ...
                if self.game_state == 'ready':
                    if per_cell_var.get() not in [self.per_cell, -1] and not self.game_origin:
                        ...
                        self.create(self.mines, overwrite=1)
                        b.invoke()
                    if first_success_var.get() and self.final_grid[coord] != 0 and not self.game_origin: #First success criteria.
