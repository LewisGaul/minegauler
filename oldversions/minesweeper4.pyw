...
Stop clicking clicked cells doing anything.
Add zoom (Window menu).
Add option to double click and drag to click multiple cells.
Change replay(/new) action.
...
default_settings = {'per_cell': 1, 'first_success': False, 'mines': 99, 'detection': 1, 'shape': (16, 30), 'difficulty': 'e', 'lives': 1, 'drag_click': 'off'}
...
detection_mines = {
            ...
            ('b', 2): 5, ('b', 2.2): 4, ('b', 2.8): 3, ('b', 3): 3,
            ('i', 0.5): 40, ('i', 1): 40, ('i', 1.5): 35, ('i', 1.8): 30,
            ('i', 2): 30, ('i', 2.2): 22, ('i', 2.8): 18, ('i', 3): 15,
            ('e', 0.5): 99, ('e', 1): 99, ('e', 1.5): 80, ('e', 1.8): 60,
            ('e', 2): 60, ('e', 2.2): 50, ('e', 2.8): 40, ('e', 3): 35}
...
class Game(Minefield):
    def __init__(self, play=True):
        ...
        try:
            for d in ['Beginner', 'Intermediate', 'Expert']:
                with open(directory + '\\' + d + r'\highscores.txt', 'r') as f:
    ...
    def play(self, mines='default', first_success=False):
        #Variables only with meaning during mainloop():
        self.grid = -np.ones(self.shape, int)
        self.score = None
        self.leftbuttondown, self.rightbuttondown = False, False
        self.blockclick, self.highscore_added = False, False
        self.coord_clicked = {'left':None, 'right':None}
        self.original_coord = None
        self.grid_was_created = False
        delete_attrs = []
        ...
        def show_info():
            ...
            if self.score:
                info += "\nYou completed it in {:.2f} seconds, with 3bv/s of {:.2f}.".format(self.score+0.005, self.get_3bv()/self.score+0.005)
            Label(info_window, text=info, font=('Times', 11, 'bold')).pack()

        def show_highscores(settings=None, flagging=None, window=None):
            if len(root.children) > 3 and not window:
                self.focus.focus_set()
                return
            if not settings:
                if not self.start:
            ...
            if not window:
                self.focus = highscores_window = Toplevel(root)
                highscores_window.title('Highscores')
                #highscores_window.resizable(False, False)
            else:
                self.focus = highscores_window = window
            ...
            intro = "{1} highscores for {f}{d} with:\nMax per cell = {2}, Detection = {3}, Drag = {4}\n{L}".format(*settings, d=dict(difficulties_list)[settings[0]].lower(), f='non-flagged' if flagging == 'NF' else ('flagged ' if flagging == 'F' else ''), L='Lives = %s\n' % self.lives if self.lives > 1 else '')
            Label(highscores_window, text=intro, font=('times', 11, 'bold')).grid(row=0, column=0, columnspan=len(headings)+1)
            for i in headings:
                Label(highscores_window, text=i.replace(' ', '\n')).grid(row=1, column=headings.index(i)+1)
            ...
            if settings in self.highscores:
                for index in range(len(self.highscores[settings])):
                    ...
                    index = None
            highscores = [] if settings not in self.highscores else [d for d in self.highscores[settings] if not flagging or d['Flagging'] == flagging][:10]
            if self.visitor:
                highscores = [d for d in highscores if 'siwel' in d['Name'].lower()]
            def set_name(event):
                name = event.widget.get()
                self.highscores[settings][index]['Name'] = name
                row = event.widget.grid_info()['row']
                event.widget.destroy()
                Label(highscores_window, text=name, font=('times', 9, 'bold')).grid(row=row, column=1)
                self.focus = highscores_window
                if settings[1] == 'Time':
                ...
                with open(directory + '\\' + dict(difficulties_list)[self.difficulty] + r'\highscores.txt', 'r') as f:
                ...
                with open(directory + '\\' + dict(difficulties_list)[self.difficulty] + r'\highscores.txt', 'w') as f:
            ...
            for d in highscores:
                ...
                Label(highscores_window, text=row-1, font=font).grid(row=row, column=0)
                col = 1
                for i in headings:
                    if i == 'Name' and not d[i] and d['Mines grid'] == self.mines_grid.tolist():
                        self.focus = e = Entry(highscores_window)
                        e.grid(row=row, column=col)
                        e.bind('<Return>', set_name)
                    else:
                        Label(highscores_window, text=d[i], font=font).grid(row=row, column=col)
                    col += 1
                row += 1

            lower_frame = Frame(highscores_window)
            lower_frame.grid(row=12, column=0, columnspan=len(headings)+1)
            def change_flagging():
                for w in highscores_window.children.values():
                    w.destroy()
                flagging = None if flagging_var.get() == 'None' else flagging_var.get()
                show_highscores(settings, flagging, window=highscores_window)
            ...
            def change_type():
                ...
                for w in highscores_window.children.values():
                    w.destroy()
                show_highscores(tuple(new_settings), flagging, window=highscores_window)
            Button(lower_frame, padx=10, bd=3, text='Time / 3bv/s', font=('times', 10, 'bold'), command=change_type).pack(side='top')
            # highscores_window.bind('<Return>', lambda event: highscores_window.destroy) #Doesn't work...
            self.focus.focus_set()
        ...
        def change_shape():
            ...
            if self.difficulty == 'c':
                if len(root.children) > 3:
                    if self.shape in shape_difficulty:
                        #Change radiobutton back.
                        difficulty_var.set(shape_difficulty[self.shape])
                    self.focus.focus_set()
                    return
                def get_mines(event):
                    ...
                    if shape in shape_difficulty:
                        ...
                        d = self.detection - 1
                        mines.set(int((0.1497*d**3 - 0.338*d**2 - 0.2291*d + 1)*(shape[0]*shape[1]*0.2)))
        ...
        def set_highscore(htype='Time', entry=None):
            self.highscore_added = True
            if self.difficulty != 'c' and not self.grid_was_created:
                if entry:
                    flagging = entry['Flagging']
                else:
                    flagging = 'F' if ((self.grid % 9 == 0) * (self.grid < 0)).any() else 'NF'
                    entry = {
                        'Name': "",
                        'Time': "{:.2f}".format(self.score+0.005),
                        '3bv': self.get_3bv(),
                        '3bv/s': "{:.2f}".format(self.get_3bv()/self.score+0.005),
                        'Flagging': flagging,
                        'Date': tm.asctime(),
                ...
                elif htype == '3bv/s' and not self.visitor:
                    if settings not in self.highscores:
                        self.highscores[settings] = []
        ...
        def leftclick(coord=None, actual_click=True):
            if not coord:
                coord = self.coord_clicked['left']
            elif coord == self.coord_clicked['left']:
                actual_click = True
            b = buttons[coord]
            if actual_click:
                ...
                if self.rightbuttondown:
                    ...
                    combinedclick(self.coord_clicked['right'])
                    return
                elif drag_click_var.get() == 'single' and self.final_grid[coord] >= 0 and (self.start or self.score):
            ...
            if create_var.get() and self.grid[coord] >= -1:
                ...
                if not self.start and (self.grid == -1).all(): #If first click
                    if per_cell_var.get() not in [self.per_cell, -1] and not self.grid_was_created:
                        self.per_cell = per_cell_var.get()
                        self.create(self.mines, overwrite=1)
                        b.invoke()
                    if first_success_var.get() and self.final_grid[coord] != 0: #First success criteria.
                        if not self.final_grid.all():
                ...
                if self.grid[coord] == -1:
                    ...
                    if (np.where(self.grid < 0, -9, self.grid) == np.where(self.final_grid < 0, -9, self.final_grid)).all():
                        self.score = tm.time() - self.start
                        print "You won in %s seconds! %s" % (round(self.score+0.05, 1), self)
                        mines_var.set("000")
                        self.start = 0
                        timer_label['fg'] = 'red'
                        for c in buttons:
        ...
        def rightdown(event):
            ...
            self.coord_clicked['right'] = coord
            self.rightbuttondown = True
            self.blockclick = False
            # combinedclick() will be implemented.
        ...
        def leftdown(event):
            ...
            self.blockclick = False
            self.coord_clicked['left'] = coord
            c = self.coord_clicked['right']
            if self.rightbuttondown and self.grid[c] >= 0:
        ...
        def rightrelease(event):
            coord = self.coord_clicked['right']
            ...
            if self.leftbuttondown:
                ...
                reset_button(buttons[coord], buttontexts[buttons[coord]])
                self.grid[coord] = -1
            if self.start and self.leftbuttondown and self.coord_clicked['left'] != coord:
                buttons[self.original_coord]['relief'] = 'sunken'

        def leftrelease(event):
            coord = tuple(map(int, event.widget.bindtags()[0].split()))
            self.leftbuttondown = False
            #Avoid running it twice. Doesn't implement when moving away from original button.
            if (self.coord_clicked['left'] != coord and not(drag_click_var.get() == 'single' and self.start)) or self.rightbuttondown:
                try:
                    leftclick()
                except KeyError:
                    pass #Not sure why this error is being produced occasionally when the shape is changed - either coord too big or None.
            elif self.coord_clicked['left'] != coord:
                buttons[self.original_coord]['relief'] = 'sunken'
            if self.score and self.drag_click == 'single' and not self.highscore_added:
                leftclick(self.original_coord)
                set_highscore()
                self.grid = np.zeros(self.shape)
        ...
        def motion(leftorright):
            def action(event):
                ...
                if c in [(u, v) for u in range(self.shape[0]) for v in range(self.shape[1])] and c != self.coord_clicked[leftorright]:
                    if self.rightbuttondown and self.leftbuttondown:
                        if drag_click_var.get() == 'single' and self.grid[coord] >= 0:
                            return
                        for c1 in self.get_neighbours(c, self.detection, 1) - self.get_neighbours(self.coord_clicked['right'], self.detection, 1):
                            if self.grid[c1] == -1:
                                buttons[c1]['relief'] = 'sunken'
                        for c1 in self.get_neighbours(self.coord_clicked['right'], self.detection, 1) - self.get_neighbours(c, self.detection, 1):
                            if self.grid[c1] == -1:
                                buttons[c1]['relief'] = 'raised'
                    elif leftorright == 'left' and not self.blockclick:
                        if self.grid[self.coord_clicked['left']] == -1:
                            buttons[self.coord_clicked['left']]['relief'] = 'raised'
                        elif drag_click_var.get() == 'single' and self.coord_clicked['left'] == coord and (self.start or not self.grid.any()):
                            leftclick(coord)
                        if self.grid[c] == -1 and not self.score:
                            if drag_click_var.get() == 'single' and self.start:
                                leftclick(c, actual_click=False)
                            else:
                                buttons[c]['relief'] = 'sunken'
                    if self.rightbuttondown:
                        self.coord_clicked['right'] = c
                    if self.leftbuttondown:
                        self.coord_clicked['left'] = c
            return action
